...
export async function envOptionSettings(
  resourceNamePrefix: string[],
  instanceProfile: iam.IInstanceProfile,
  securityGroups: ec2.SecurityGroup[],
  vpc: ec2.IVpc
): Promise<eb.CfnEnvironment.OptionSettingProperty[]> {
  ...
  const result = uniqConf
    ...
    .filter((setting, index, arr) => {
      ...
      return !arr
        .slice(0, index - 1)
        .some((dup) => setting.optionName == dup.optionName && setting.resourceName == dup.resourceName);
    });
