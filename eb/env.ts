...
function createSecurityGroups(stack: Stack, vpc: ec2.IVpc): ec2.SecurityGroup[] {
  const lbSecurityGroup = new SecurityGroupDirector(SecurityGroupBuilder).constructWebSecurityGroup(stack, "LbSecurityGroup", vpc);
  const webSecurityGroup = new SecurityGroupDirector(SecurityGroupBuilder).constructWebSecurityGroup(stack, "WebSecurityGroup", vpc);
